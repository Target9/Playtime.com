...
function loadPersonData(personName) {
    let htmlContent = `<h3>Manage ${personName}</h3>`;
    ...
    htmlContent += `<button id="updateScriptButton" onclick="updatePythonScript()">Update Script</button>`;
    document.getElementById('content').innerHTML = htmlContent;
}
...
function updatePythonScript() {
    ...
    .catch(error => {
        ...
        alert('Failed to update script. Please try again later.');
    });
}
